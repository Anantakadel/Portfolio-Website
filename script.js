...
function validateSubjectField(subject) {
  const isValid = subject && subject.length >= 3;
  if (!isValid) {
...
nameInput.addEventListener("blur", () => {
  validateNameField(nameInput.value.trim());
});
...
messageInput.addEventListener("blur", () => {
  validateMessageField(messageInput.value.trim());
});

// Subject validation
if (subjectInput) {
  subjectInput.addEventListener("blur", () => {
    validateSubjectField(subjectInput.value.trim());
  });
}
...
contactForm.addEventListener("submit", (e) => {
  ...
  const isSubjectValid = validateSubjectField(subject);
